...
from OCPI.source.utils import Format, TariffDimensionType, Price
...
def test_tariff():
    ...
    assert (
        tariff.elements[0].price_components[0].type == TariffDimensionType.ENERGY.value
    )
    assert tariff.elements[0].price_components[0].price == 0.30
    print(tariff.last_updated)
...
def test_tariff_json():
    ...
    tariff = TariffObject.from_json(json_data)
    ...
    assert (
        TariffObject.from_json(tariff.to_json(simple=True)).to_json()["elements"]
        == json_data["elements"]
    )
    assert json.dumps(
...
def test_to_text():
    ...
    for example in examples_data:
        tariff = TariffObject.from_json(example)
        text += tariff.to_text() + "\n"
