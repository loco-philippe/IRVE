...
- les données cumulables dont le passage à l'échelle se traduit par un cumul (somme). 

```{admonition} Exemple
C'est le cas du nombre de session dont la valeur annuelle peut être calculée comme la somme des sessions mensuelles elles-mêmes calculées comme la somme des sessions quotidiennes,
```

- les données ajustables dont le passage à l'échelle se traduit par un ensemble de valeurs (moyenne, maxi, mini, dernière). 
Par exemple, la puissance moyenne installée de l'année peut être calculée à partir de la moyenne des puissances installées du mois elles-mêmes calculées à partir de la moyenne des puissances installées quotidiennes.
...
L'opérateur 'oper1' dispose pour le mois 'xxxxxxx' de :

- 75 stations en fin de mois, 
- 60 stations en moyenne sur le mois,
- d'un nombre de stations variant entre 52 et 75
